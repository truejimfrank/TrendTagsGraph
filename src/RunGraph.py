...
def print_sel_shape(txt_pattern):
    ...
    print(df.loc[df["tags"].str.contains(txt_pattern)].shape[0])
...
# Education top20 tags
    tags_list_edu = TagsResearch.create_tags_list(df.loc[df["category_name"] == "Education"])
    bar_20 = pd.Index(tags_list_edu).value_counts().reset_index(name="tag_num").head(20)
...
# topically selected wordclouds
    ...
    # printing pattern selection and # of rows
